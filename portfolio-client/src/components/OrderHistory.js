...
const OrderHistory = () => {
  ...
  const { user } = useAuth();
  const { getUserOrders, getOrderStatus, getPendingPaymentOrders } = useOrders();
  ...
  const pendingPaymentOrders = getPendingPaymentOrders(user.id);
  const loading = false;
  ...
  const handlePaymentComplete = (order) => {
    // Order will be updated by the payment context
    setShowUpiPayment(false);
    setSelectedOrderForPayment(null);
    // Refresh the page or update the order list
    window.location.reload();
  };

  const handlePaymentFailed = (order) => {
    setShowUpiPayment(false);
    setSelectedOrderForPayment(null);
    ...
    switch (selectedTab) {
      ...
      case 2: // Active orders
        return orders.filter(order => 
          order.status !== 'pending' && getOrderStatus(order) !== 'delivered'
        );
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 4 }}>
          <Tabs value={selectedTab} onChange={handleTabChange} centered>
            ...
            <Tab 
              label={`Pending Payment (${pendingPaymentOrders.length})`} 
              icon={pendingPaymentOrders.length > 0 ? <Warning color="error" /> : null}
              iconPosition="end"
            />
            <Tab label={`Active (${orders.filter(order => order.status !== 'pending' && getOrderStatus(order) !== 'delivered').length})`} />
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                      <Typography variant="body2">
                        {order.items.length} item{order.items.length !== 1 ? 's' : ''} • Total: ₹{order.total.toFixed(0)}
                      </Typography>
                    ...
                    {isPendingPayment && (
                      <Alert severity="warning" sx={{ mb: 2 }}>
                        <Typography variant="body2">
                          <strong>Payment Pending:</strong> Complete your UPI payment to confirm this order.
                        </Typography>
                        <Button
                          variant="contained"
                          size="small"
                          onClick={() => handleRetryPayment(order)}
                          sx={{ mt: 1, backgroundColor: '#2E7D32' }}
                        >
                          Complete Payment
                        </Button>
                      </Alert>
